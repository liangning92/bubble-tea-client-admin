  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4">
        {loading ? (
          <div className="py-20 text-center animate-pulse text-slate-300 font-bold">{t('syncingRecords')}</div>
        ) : filtered.length === 0 ? (
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                   <div className="flex items-center gap-2 mb-1">
                      ...
                      {r.status === 'appealed' && <span className="px-4 py-0.5 bg-orange-500 text-white text-[8px] font-black rounded animate-pulse">{t('pendingAppeal')}</span>}
                   </div>
              ...
              <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
                ...
                <div className="flex gap-2">
                   {r.status === 'appealed' ? (
                     <>
                        <button 
                          onClick={() => setResolveModal({ id: r.id, action: 'revoked', title: t('revokeRewardTitle') })}
                          className="px-4 py-3 bg-emerald-600 text-white text-[14px] font-black rounded-xl shadow-lg active:scale-95 transition-all"
                        >
                          {t('approve')}/{t('revoke')}
                        </button>
                        <button 
                          onClick={() => setResolveModal({ id: r.id, action: 'rejected', title: t('rejectAppealTitle') })}
                     ...
                     <button 
                        onClick={() => setResolveModal({ id: r.id, action: 'revoked', title: t('directRevokeTitle') })}
      ...
      {resolveModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm">
           <div className="bg-white max-w-md w-full rounded-[40px] shadow-2xl p-8 animate-in zoom-in-95">
              ...
              <p className="text-sm text-slate-500 mb-6 font-medium">{t('optionalNote')}:请输入处理备注或理由：</p>
              <textarea 
              ...
              <div className="flex gap-4 mt-8">
                 <button onClick={() => setResolveModal(null)} className="flex-1 py-3 bg-slate-100 text-slate-500 font-bold rounded-2xl">{t('cancel')}</button>
                 <button onClick={() => handleResolve(resolveModal.id, resolveModal.action)} className="flex-1 py-3 bg-slate-900 text-white font-black rounded-2xl shadow-lg transition-all active:scale-95">{t('confirmAction')}</button>
              </div>
           </div>
        </div>
      )}
