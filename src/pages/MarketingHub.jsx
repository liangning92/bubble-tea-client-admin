...
export default function MarketingHub() {
  ...
  return (
    <div className="animate-soft space-y-5 focus:outline-none text-slate-900 pb-20">
    ...
    <div className="space-y-5 animate-soft text-slate-900">
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={i} className={`card-premium !p-6 border-slate-50 !rounded-[40px] ${!item.active && 'opacity-50 grayscale'}`}>
...
function MarketingAssets({ hideHeader }) {
  return (
    <div className="space-y-5 animate-soft text-slate-900">
       <div className="flex flex-col md:flex-row justify-between items-end gap-6">
          <h3 className="text-xl font-black text-slate-900 uppercase">{t('brandVisualLibrary')}</h3>
          <button className="btn-premium active !bg-slate-900 !text-white !px-10 !h-14 border-none shadow-xl !rounded-[20px] text-[13px] font-black uppercase tracking-widest">
             上传品牌素材
          </button>
       </div>
          ...
          {[1,2,3,4].map(i => (
             <div key={i} className="card-premium !p-0 overflow-hidden border-slate-50 group hover:shadow-2xl transition-all !rounded-[32px]">
                ...
                <div className="p-6 space-y-4 bg-white text-center">
                   <p className="text-[14px] font-black text-slate-800 truncate">{t('brandActivity')}_0{i}.png</p>
                   <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest">{t('assetSize')}: 1080x1080 / 2.4MB</p>
                </div>
             </div>
          ))}
       </div>

       <div className="p-16 rounded-[48px] bg-slate-900 text-white flex flex-col items-center justify-center text-center space-y-4">
          ...
          <div className="max-w-md mx-auto space-y-4">
             <h4 className="font-black uppercase tracking-widest text-[16px] text-orange-400">{t('cdnSyncReady')}</h4>
